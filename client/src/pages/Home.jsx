...
const Home = () => {
  ...
  useEffect(() => {
    const fetchTestimonials = async () => {
      try {
        setLoading(true);
        const response = await API.get(TESTIMONIAL_PATHS.ALL);
        
        // Check if the response has the expected format
        if (response && response.data && Array.isArray(response.data.data)) {
          // Map the API response to the format expected by the component
          const formattedTestimonials = response.data.data.map(item => ({
            quote: item.content,
            author: item.name,
            role: item.role
          }));
          ...
          console.warn('API returned unexpected testimonial format:', response);
  ...
  const handleSubmitTestimonial = async (e) => {
    ...
    try {
      // Ensure testimonials is an array before spreading
